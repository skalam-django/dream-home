	  ...
	  class AnalogTank {
	    ...
	    addThresholdMarkers() {
	      ...
	      this.thresholds.forEach( (threshold, i) => {
	        ...
	        var top = 90;
	        if (threshold.level=='High'){
	        	top = 10;
	        }
	        let tooltip = d3.select(this.container[0]).append('div')
	          .datum({ status: threshold.status, value: this.textFormatter(threshold.value), volume: threshold.volume, level: threshold.level })
	          .html(function(d) { return '<div>Pump => ' + d.status + '</div>' + '<div>Height => ' + d.value + '</div>' + '<div>Volume => ' + d.volume + '</div>' + '<div>Level => ' + d.level + '</div>'; })
	          .attr('id', `tooltip-${id}`)
	          .style('position', 'absolute')
	          .style('right', `${this.markerLabelWidth*2}px`)
	          .style('top', `${pixelPosition + this.innerHeight/2 - top}px`)
	          .style('padding', '8px')
